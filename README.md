...
## Публикация демо на Render

В продакшене оба слоя работают одним процессом: Express отдаёт API по `/api`
и раздаёт собранный фронтенд из `frontend/dist`, поэтому прокси Vite не нужен.
...
2. На https://render.com — **New → Blueprint**, выбрать репозиторий.
   Render прочитает `render.yaml` и создаст бесплатный web-сервис:
   сборка `npm run build`, запуск `npm start`.
3. Порт подставляет сам Render через переменную `PORT`, настраивать её не нужно.

Демо будет доступно по адресу вида `https://vodokanal-iot-pilot.onrender.com`.

На бесплатном тарифе сервис засыпает после 15 минут простоя — первый заход
после паузы занимает около минуты. Данные при засыпании сбрасываются к моковым,
потому что база не используется.
